           ...
           # realsense exposure
            ...
            while True:
                ...
                try:
                    ...
                    frame_latency = 1/30
                    precise_wait(eval_t_start - frame_latency,
                                 time_func=time.time)
                    print("Started!")
                    iter_idx = 0
                    ...
                    while True:
                        # calculate timing
                        t_cycle_end = t_start + \
                            (iter_idx + steps_per_inference) * dt
                        ...
                        print(
                            f'Obs latency {time.time() - obs_timestamps[-1]}')
                        ...
                        with torch.no_grad():
                            ...
                            action = result['action'][0].detach().to(
                                'cpu').numpy()
                        ...
                        if delta_action:
                            ...
                            this_target_poses = np.expand_dims(
                                this_target_pose, axis=0)
                        else:
                        ...
                        term_pose = np.array(
                            [3.40948500e-01,  2.17721816e-01,  4.59076878e-02,  2.22014183e+00, -2.22184883e+00, -4.07186655e-04])
                        curr_pose = obs['robot_eef_pose'][-1]
                        dist = np.linalg.norm(
                            (curr_pose - term_pose)[:2], axis=-1)
                        if dist < 0.03:
                            # in termination area
